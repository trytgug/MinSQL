...
openai.api_key = " "   ##Adding your openai apikey


def parse_option():
    parser = argparse.ArgumentParser("command line arguments for recall columns")
    parser.add_argument("--input_recalled_tables_path", type=str, default="./generate_datasets/table_recall.json")
    ...
    parser.add_argument("--output_recalled_columns_path", type=str, default="./generate_datasets/column_recall.json")
...
1 - Find the related columns and values for generating SQL queries for each question based on the database schema according to the question order. 
1 - For each selected columns, you should find a clause in the question as the evidence. 
2 - Output a JSON object that contains all the selected columns in each table according to the evidence. The format should be like: 
{
    "column_1": evidence_1, 
    "column_2": evidence_2,
    "column_3": evidence_3,
